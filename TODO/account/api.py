...
class ListCustomerApi(APIView):
    ...
    def get(self, request,*args,  **kwargs):
        serializer = GetCustomerDataSerializer(Customer.objects.values(),many=True)
       # print(serializer)
        return Response({
...
class TargetCustomerApi(APIView):
    ...
    def get(self,request,*args,  **kwargs):
        serializer_male=Customer.objects.filter(age__gte=18, age__lte=50, gender='Male', country__in=["US", "IN", "EN", "AU", "FR"])
        serializer_female=Customer.objects.filter(age__gte=15, age__lte=45, gender='Female',country__in=["US", "IN", "EN", "AU", "FR"])
        data_set = GetCustomerDataSerializer(serializer_male,many=True)

        datasetfemale=GetCustomerDataSerializer(serializer_female,many=True)
        return Response(
            {
                "Target-Customer-Male":data_set.data,
